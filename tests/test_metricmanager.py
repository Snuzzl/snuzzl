...
def test_update_metric_value_nonexistent_metric_id(mm, db_mock):
    user_id = 1
    nonexistent_metric_id = 999
    value = 20
    class FakeMetricValue:
        def __init__(self, user_id, metric_id, value):
            self.user_id = user_id
            self.metric_id = metric_id
            self.value = value
    fake_record = FakeMetricValue(user_id, nonexistent_metric_id, value)
    mm._db.create_record.return_value = fake_record
    res = mm.update_metric_value(user_id=user_id, metric_id=nonexistent_metric_id, value=value)
    assert res is fake_record
    mm._db.create_record.assert_called_once()
    args, kwargs = mm._db.create_record.call_args
    assert args[0] == mm.metric_value_table
    assert kwargs["user_id"] == user_id
    assert kwargs["met_id"] == nonexistent_metric_id
    assert kwargs["metval_val"] == value
    from datetime import date
    assert kwargs["metval_date"] == date.today()

@pytest.mark.xfail(reason="non-existent user id should raise or DB FK error", strict=False)
def test_update_metric_value_nonexistent_user_id(mm, db_mock):
    ...
    class FakeMetricValue:
        def __init__(self, user_id, metric_id, value):
            ...
            self.metric_id = metric_id
            self.value = value
    fake_record = FakeMetricValue(nonexistent_user_id, metric_id, value)
    mm._db.create_record.return_value = fake_record
    res = mm.update_metric_value(
    ...
    args, kwargs = mm._db.create_record.call_args
    assert args[0] == mm.metric_value_table
    assert kwargs["user_id"] == nonexistent_user_id
    assert kwargs["met_id"] == metric_id
    assert kwargs["metval_val"] == value
    from datetime import date
    assert kwargs["metval_date"] == date.today()
...
def test_update_metric_value_wrong_type_for_value(mm, db_mock):
    ...
    class FakeMetricValue:
        def __init__(self, user_id, metric_id, value):
            ...
            self.metric_id = metric_id
            self.value = value
    fake_record = FakeMetricValue(user_id, metric_id, wrong_value)
    mm._db.create_record.return_value = fake_record
    res = mm.update_metric_value(
        user_id=user_id,
        metric_id=metric_id,
    ...
    assert kwargs["user_id"] == user_id
    assert kwargs["met_id"] == metric_id
    assert kwargs["metval_val"] == wrong_value
    from datetime import date
    assert kwargs["metval_date"] == date.today()

@pytest.mark.xfail(reason="null inputs should raise", strict=False)
def test_update_metric_value_null_inputs(mm, db_mock):
    user_id = None
    metric_id = None
    value = None
    class FakeMetricValue:
    ...
    assert kwargs["met_id"] is None
    assert kwargs["metval_val"] is None
    from datetime import date
    assert kwargs["metval_date"] == date.today()
